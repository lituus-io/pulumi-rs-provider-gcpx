...
use crate::workflow_template::{
    build_ingest_bq_args, build_ingest_gcs_args, definition_fingerprint,
...
pub async fn diff_ingest_job<C: SchedulerOps>(
    ...
    let current_fingerprint = definition_fingerprint(&generate_ingest_workflow(&new_inputs));
...
pub async fn create_ingest_job<C: SchedulerOps>(
    ...
    if req.preview {
        let state = DataprocJobState {
            ...
            definition_fingerprint: definition_fingerprint(&generate_ingest_workflow(&inputs)),
        };
...
mod tests {
    ...
    /// A change to how the workflow is generated must reach a stack whose
    /// inputs have not moved.
    ///
    /// The definition is derived state — none of it is an input — so before the
    /// fingerprint existed, correcting the generator left every deployed stack
    /// running the old definition until something unrelated happened to change.
    /// That is how three broken batch submissions survived: `pulumi up` kept
    /// reporting "unchanged".
    #[tokio::test]
    async fn a_changed_definition_is_noticed_even_when_inputs_are_identical() {
        let client = MockSchedulerClient::new();
        let inputs = gcs_oracle_struct();

        // Same inputs on both sides, but the stored fingerprint is from an
        // older generator.
        let mut olds = inputs.clone();
        olds.fields.insert(
            "definitionFingerprint".to_owned(),
            prost_string("0000000000000000"),
        );
        ...
        assert_eq!(
            ...
            "a stale definition was reported as no change"
        );

        // And the current fingerprint must settle: no diff when it matches.
        let parsed = parse_ingest_inputs(&inputs).unwrap();
        let current = definition_fingerprint(&generate_ingest_workflow(&parsed));
        let mut olds_ok = inputs.clone();
        olds_ok
            .fields
            .insert("definitionFingerprint".to_owned(), prost_string(&current));
        let resp = diff_ingest_job(
            &client,
            pulumirpc::DiffRequest {
                olds: Some(olds_ok),
        ...
        assert_eq!(
            ...
            "a current definition was reported as a change"
        );
    }
    ...
    async fn test_diff_ingest_no_changes() {
        ...
        let fp =
            definition_fingerprint(&generate_ingest_workflow(&parse_ingest_inputs(&s).unwrap()));
        s.fields
